...
\begin{document}
	...
	\begin{itemize}
		...
		\item Por ultimo el calculo y diseño de los filtros debe apegarse a los requerimientos mencionados en el cuadro anterior.
	\end{itemize}
	...
	La biblioteca \texttt{CMSIS-DSP}, desarrollada por ARM, proporciona funciones optimizadas para realizar procesamiento digital de señales en microcontroladores con núcleo Cortex-M. En este trabajo se utilizó su soporte para aritmética de punto fijo en formato \texttt{Q15}, adecuado para implementar filtros FIR de manera eficiente sobre la plataforma STM32F446RE. Este formato representa las muestras y los coeficientes como enteros de 16 bits con signo, permitiendo reducir el costo computacional respecto de una implementación en punto flotante.
	
	La implementación de un filtro FIR con CMSIS-DSP parte del diseño previo de los coeficientes del filtro. Una vez calculados, los coeficientes se escalan y cuantizan al formato \texttt{Q15}, cuidando que sus valores permanezcan dentro del rango representable. Para cada tipo de filtro utilizado en el programa, se definió un arreglo de coeficientes y un buffer de estado asociado. El filtro queda descripto mediante una estructura del tipo \texttt{arm\_fir\_instance\_q15}, que almacena la cantidad de coeficientes, el puntero al arreglo de coeficientes y el puntero al buffer de estado.
	
	La inicialización se realiza mediante la función \texttt{arm\_fir\_init\_q15}. Esta función recibe la estructura del filtro, el número de coeficientes, el arreglo de coeficientes, el buffer de estado y la cantidad de muestras que se procesarán por bloque. El buffer de estado debe tener una longitud suficiente para conservar las muestras anteriores necesarias para la convolución, típicamente \texttt{numTaps + blockSize - 1}. A diferencia de una implementación IIR, no se utilizan etapas biquad ni términos de realimentación; la salida depende únicamente de la muestra actual y de muestras pasadas de la entrada.
	
	Una vez inicializado, el filtrado se aplica con la función \texttt{arm\_fir\_q15}. Esta función toma como argumentos la instancia del filtro, el bloque de muestras de entrada en formato \texttt{Q15}, el bloque de salida y la cantidad de muestras a procesar. Internamente realiza la convolución FIR y actualiza el estado del filtro, permitiendo procesar sucesivos bloques provenientes del ADC sin perder continuidad temporal entre ellos.
	
	En el programa desarrollado, las muestras adquiridas por el ADC se convierten primero desde el rango de 12 bits al formato \texttt{Q15}. Luego, según el filtro seleccionado, se ejecuta la instancia FIR correspondiente mediante \texttt{arm\_fir\_q15}. Finalmente, las muestras filtradas se reconvierten al rango del DAC para su visualización en el osciloscopio. Respetar esta secuencia de conversión, inicialización y procesamiento es fundamental para evitar saturaciones, errores de escala o discontinuidades entre bloques.
	...
	\subsection{Criterio de diseño FIR en MATLAB}
	
	El diseño final de los filtros se realizó con MATLAB. Para ello se utilizó el script
	\path{DesingFiltroFIR_v2.m}, ubicado en la carpeta \path{PDS_Lab_2}.
	La implementación definitiva utiliza filtros FIR, por lo que el cálculo se centra
	en obtener los coeficientes de la respuesta al impulso que luego serán usados
	por la función \texttt{arm\_fir\_q15} de la biblioteca CMSIS-DSP.
	
	El script define las frecuencias de muestreo utilizadas por el sistema y diseña una familia de filtros para cada una de ellas. Se contemplan filtros pasa bajo, pasa alto, pasa banda y rechaza banda. Para el pasa bajo se emplea un diseño FIR equiripple, mientras que para los restantes filtros se utiliza un diseño FIR por mínimos cuadrados restringidos mediante \texttt{fircls}. En todos los casos se parte de las especificaciones de frecuencia y atenuación requeridas, se fija la cantidad de coeficientes y se obtiene el vector \texttt{Numerator}, que corresponde directamente a los coeficientes del filtro FIR.
	
	En los filtros pasa bajo y pasa alto se incorpora además una corrección de la frecuencia de corte. Para ello, el script calcula la respuesta en frecuencia del filtro diseñado, localiza el punto de \(-3\,\mathrm{dB}\) y rediseña el filtro con una frecuencia corregida. De esta forma se busca que la frecuencia efectiva medida sobre la respuesta del filtro coincida mejor con la especificación deseada.
	
	\subsection{Cuantización de coeficientes en Q15}
	
	Una vez obtenidos los coeficientes en punto flotante, se analiza su comportamiento en formato \texttt{Q15}. Este paso es importante porque la implementación en el microcontrolador no utiliza directamente los coeficientes en doble precisión de MATLAB, sino una representación entera de 16 bits compatible con CMSIS-DSP. La cuantización se realiza escalando por \(2^{15}\), limitando los valores al rango de \texttt{int16} y reconstruyendo la respuesta en frecuencia a partir de esos coeficientes cuantizados.
	
	El script utiliza \texttt{freqz} para comparar la magnitud y la fase de los filtros ya cuantizados. Esta verificación permite observar el efecto real de la cuantización sobre cada filtro antes de exportar los coeficientes al proyecto en C.
	
	\subsection{Exportación de coeficientes para CMSIS-DSP}
	
	El mismo archivo de MATLAB incluye una rutina de exportación a un archivo de cabecera. Para cada filtro se obtiene el vector de coeficientes, se convierte a \texttt{Q15} mediante \texttt{fi}, \texttt{numerictype} y \texttt{fimath}, y se escriben definiciones en C con el número de taps y el arreglo \texttt{const q15\_t} correspondiente. Este formato es el que luego se utiliza en el firmware para inicializar las estructuras \texttt{arm\_fir\_instance\_q15} mediante \texttt{arm\_fir\_init\_q15}.
	
	A continuación se muestra un extracto representativo del script utilizado. El archivo completo contiene el mismo procedimiento aplicado a todos los tipos de filtro y a todas las frecuencias de muestreo consideradas.
	
	\begin{lstlisting}[style=matlabstyle]
% Frecuencias de muestreo utilizadas
Fs = [8000, 16000, 22000, 44000, 48000];

%% Diseno FIR pasa bajo
FPB_Fc  = 3600;
FPB_Ast = 30;
FPB_Ap  = 0.5;
N_FPB_8K = 79;

FPB_8K_Specs = fdesign.lowpass('N,Fc,Ap,Ast', ...
    N_FPB_8K, FPB_Fc, FPB_Ap, FPB_Ast, Fs(1));
FPB_8K = design(FPB_8K_Specs, 'equiripple');

Fc_corr = fcParaQueF3dBSea(FPB_8K.Numerator, Fs(1), FPB_Fc);
FPB_8K_spec_corr = fdesign.lowpass('N,Fc,Ap,Ast', ...
    N_FPB_8K, Fc_corr, FPB_Ap, FPB_Ast, Fs(1));
FPB_8K = design(FPB_8K_spec_corr, 'equiripple');

%% Diseno FIR pasa alto, pasa banda y rechaza banda
FPA_Fc = 400;
FPA_Ast = 30;
FPA_Ap = 1.5;
N_FPA_8K = 79;
FPA_8K_Specs = fdesign.highpass('N,Fc,Ast,Ap', ...
    N_FPA_8K, FPA_Fc, FPA_Ast, FPA_Ap, Fs(1));
...
FPBD_Ast2 = 30;
N_FPBD_8K = 79;
FPBD_8K_Specs = fdesign.bandpass('N,Fc1,Fc2,Ast1,Ap,Ast2', ...
    N_FPBD_8K, FPBD_Fc1, FPBD_Fc2, FPBD_Ast1, FPBD_Ap, FPBD_Ast2, Fs(1));
...
FRBD_Ap2 = 2;
N_FRBD_8K = 79;
FRBD_8K_Specs = fdesign.bandstop('N,Fc1,Fc2,Ap1,Ast,Ap2', ...
    N_FRBD_8K, FRBD_Fc1, FRBD_Fc2, FRBD_Ap1, FRBD_Ast, FRBD_Ap2, Fs(1));
FRBD_8K = design(FRBD_8K_Specs, 'fircls');

%% Verificacion de respuesta con coeficientes cuantizados a Q15
Q = 15;
S = 2^Q;
INT16_MIN = -32768;
INT16_MAX =  32767;
...
for i = 1:5
    ...
    [H, w] = freqz(bq, 1, NFFT, Fs(i));
    plot(w, 20*log10(abs(H) + 1e-12));
end

%% Exportacion de coeficientes para CMSIS-DSP
T_q15 = numerictype(1, 16, 15);
...
fprintf(fid, 'const q15_t %s[%s] = {\n', coeffs_name, tap_define);
	...
	\subsection{Respuestas en frecuencia obtenidas}
	
	Las siguientes figuras muestran las respuestas en frecuencia obtenidas para las distintas familias de filtros FIR. Estas curvas permiten verificar el comportamiento de los coeficientes antes de incorporarlos al firmware y comprobar que la cuantización a \texttt{Q15} conserva la respuesta esperada para cada caso.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=1\linewidth]{pasa_bajo}
		\caption[Respuesta FIR pasa bajo en Q15]{Respuesta en frecuencia de los filtros FIR pasa bajo cuantizados en Q15}
		\label{fig:pasabajo}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\caption[Respuesta FIR pasa alto en Q15]{Respuesta en frecuencia de los filtros FIR pasa alto cuantizados en Q15}
		\label{fig:pasaalto}
	\end{figure}\newpage
	...
	\begin{figure}[H]
		...
		\caption[Respuesta FIR pasa banda en Q15]{Respuesta en frecuencia de los filtros FIR pasa banda cuantizados en Q15}
		\label{fig:pasabanda}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\caption[Respuesta FIR rechaza banda en Q15]{Respuesta en frecuencia de los filtros FIR rechaza banda cuantizados en Q15}
	...
	*/
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_bajo_8k.png}
		\caption{Respuesta en amplitud del filtro pasa bajo para una frecuencia de muestreo de 8 kHz.}
		\label{fig:pasa_bajo_8k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_bajo_16k.png}
		\caption{Respuesta en amplitud del filtro pasa bajo para una frecuencia de muestreo de 16 kHz.}
		\label{fig:pasa_bajo_16k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_bajo_22k.png}
		\caption{Respuesta en amplitud del filtro pasa bajo para una frecuencia de muestreo de 22 kHz.}
		\label{fig:pasa_bajo_22k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_bajo_44k.png}
		\caption{Respuesta en amplitud del filtro pasa bajo para una frecuencia de muestreo de 44 kHz.}
		\label{fig:pasa_bajo_44k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_bajo_48k.png}
		\caption{Respuesta en amplitud del filtro pasa bajo para una frecuencia de muestreo de 48 kHz.}
		\label{fig:pasa_bajo_48k}
	\end{figure}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_alto_8k.png}
		\caption{Respuesta en amplitud del filtro pasa alto para una frecuencia de muestreo de 8 kHz.}
		\label{fig:pasa_alto_8k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_alto_16k.png}
		\caption{Respuesta en amplitud del filtro pasa alto para una frecuencia de muestreo de 16 kHz.}
		\label{fig:pasa_alto_16k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_alto_22k.png}
		\caption{Respuesta en amplitud del filtro pasa alto para una frecuencia de muestreo de 22 kHz.}
		\label{fig:pasa_alto_22k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_alto_44k.png}
		\caption{Respuesta en amplitud del filtro pasa alto para una frecuencia de muestreo de 44 kHz.}
		\label{fig:pasa_alto_44k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_alto_48k.png}
		\caption{Respuesta en amplitud del filtro pasa alto para una frecuencia de muestreo de 48 kHz.}
		\label{fig:pasa_alto_48k}
	\end{figure}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_banda_8k.png}
		\caption{Respuesta en amplitud del filtro pasa banda para una frecuencia de muestreo de 8 kHz.}
		\label{fig:pasa_banda_8k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_banda_16k.png}
		\caption{Respuesta en amplitud del filtro pasa banda para una frecuencia de muestreo de 16 kHz.}
		\label{fig:pasa_banda_16k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_banda_22k.png}
		\caption{Respuesta en amplitud del filtro pasa banda para una frecuencia de muestreo de 22 kHz.}
		\label{fig:pasa_banda_22k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_banda_44k.png}
		\caption{Respuesta en amplitud del filtro pasa banda para una frecuencia de muestreo de 44 kHz.}
		\label{fig:pasa_banda_44k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Pasa_banda_48k.png}
		\caption{Respuesta en amplitud del filtro pasa banda para una frecuencia de muestreo de 48 kHz.}
		\label{fig:pasa_banda_48k}
	\end{figure}

	\subsection{Filtro rechaza banda}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Rechaza_banda_8k.png}
		\caption{Respuesta en amplitud del filtro rechaza banda para una frecuencia de muestreo de 8 kHz.}
		\label{fig:rechaza_banda_8k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Rechaza_banda_16k.png}
		\caption{Respuesta en amplitud del filtro rechaza banda para una frecuencia de muestreo de 16 kHz.}
		\label{fig:rechaza_banda_16k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Rechaza_banda_22k.png}
		\caption{Respuesta en amplitud del filtro rechaza banda para una frecuencia de muestreo de 22 kHz.}
		\label{fig:rechaza_banda_22k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Rechaza_banda_44k.png}
		\caption{Respuesta en amplitud del filtro rechaza banda para una frecuencia de muestreo de 44 kHz.}
		\label{fig:rechaza_banda_44k}
	\end{figure}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\textwidth]{Graficos/Rechaza_banda_48k.png}
		\caption{Respuesta en amplitud del filtro rechaza banda para una frecuencia de muestreo de 48 kHz.}
		\label{fig:rechaza_banda_48k}
	\end{figure}
